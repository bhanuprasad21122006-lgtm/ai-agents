...
ai-agents/
│
├── agents/                  # 14 specialized agents across 9 production phases
│   ├── phase1_design.py     # Game Designer, System Architect
│   ├── phase2_core.py       # Gameplay Programmer, AI Engineer, Level Designer
│   ├── phase3_arts.py       # Graphics Engineer, UI/UX Designer, Sound Engineer
│   ├── phase4_6_technical.py# Network Engineer, Asset Manager, Test Engineer
│   └── phase7_9_ops.py      # Debugging Specialist, Performance Optimizer, Live Ops
│
├── utils/                   # Shared pipeline utilities
│   ├── artifacts.py         # Universal code extraction & disk persistence
│   ├── models.py            # Dynamic Gemini model discovery & priority failover
│   ├── mcp_client.py        # Model Context Protocol bridge
│   └── validator.py         # JSON schema & phase-gate validation
│
├── docs/assets/             # Demo showcase media
│   └── demo_gameplay.webp   # Autoplaying gameplay preview for README
│
├── snake_game/              # Verified playable web game generated by the studio
│   ├── index.html
│   ├── style.css
│   └── script.js
│
├── generated_game/          # Auto-created output directory for generated runs
│   └── .gitkeep
│
├── main.py                  # Main CLI entry point (Live Demo & Full AAA Pipeline)
├── fix_models.py            # Standalone model auto-detection & .env updater
├── test_models.py           # Model validation test suite
├── requirements.txt         # Project dependencies
├── .env.example             # Environment variable template
├── run_builder.bat          # Windows quick launcher
└── build_executable.bat     # PyInstaller standalone build script
```
...
> **Note:** `GEMINI_MODEL=auto` will automatically query the Google Gemini API to select the best available model (`gemini-3.6-flash`, `gemini-3.5-flash`, etc.) with zero manual configuration.
...
When prompted, you can choose between two orchestration modes:
- **[1] Live Demo Mode (⚡ ~30s)**: Runs core agents (*Designer → Architect → Gameplay Programmer*) to build and immediately launch a playable web game (`index.html`) in your browser. Conserves daily API quota.
- **[2] Full AAA Pipeline (🔬 ~3m)**: Coordinates all 14 specialized agents across all 9 phases with deep technical GDDs, audio architectures, test plans, and ops strategies.
